...
namespace NiceHashMiner.Miners {
    ...
    public class MiningSession
    {
        ...
        {
            get { return !IsMiningEnabled || !IsConnectedToInternet || !IsProfitable; }
        }
        ...
        private bool CheckIfProfitable(double CurrentProfit, bool log = true)
        {
            ...
            var currentProfitPoints = (CurrentProfit * Globals.BitcoinUSDRate * Globals.PointsUSDRate);
            IsProfitable =
                IsMiningRegardlesOfProfit
                || !IsMiningRegardlesOfProfit && currentProfitPoints >= ConfigManager.GeneralConfig.MinimumProfit;
            if (log)
            {
                Helpers.ConsolePrint(TAG,
                    "Current Global profit: " + currentProfitPoints.ToString("F8") + " Points/Day");
                if (!IsProfitable)
                {
                    Helpers.ConsolePrint(TAG,
                        "Current Global profit: NOT PROFITABLE MinProfit " +
                        ConfigManager.GeneralConfig.MinimumProfit.ToString("F8") + " Points/Day");
                }
                else
                {
                    string profitabilityInfo = IsMiningRegardlesOfProfit
            ...
            bool log = true)
        {
            ...
            Helpers.ConsolePrint(TAG,
                String.Format("PrevStateProfit {0}, CurrentProfit {1}", PrevStateProfit, CurrentProfit));
            if (PrevStateProfit > 0 && CurrentProfit > 0)
            {
                ...
                if (percDiff < ConfigManager.GeneralConfig.SwitchProfitabilityThreshold)
                {
                    // don't switch
                    Helpers.ConsolePrint(TAG,
                        String.Format("Will NOT switch profit diff is {0}, current threshold {1}", percDiff,
                ...
                {
                    Helpers.ConsolePrint(TAG,
                        String.Format("Will SWITCH profit diff is {0}, current threshold {1}", percDiff,
                            ConfigManager.GeneralConfig.SwitchProfitabilityThreshold));
                }
            }
            ...
            //bool IsMinerStatsCheckUpdate = false;
            {
                ...
                if ((toStopGroupMiners.Values.Count > 0) || (toRunNewGroupMiners.Values.Count > 0))
                {
                    ...
                    {
                        stringBuilderPreviousAlgo.Append(String.Format("{0}: {1}, ", toStop.DevicesInfoString,
                            toStop.AlgorithmType));
                    ...
                    {
                        stringBuilderCurrentAlgo.Append(String.Format("{0}: {1}, ", toStart.DevicesInfoString,
                            toStart.AlgorithmType));
                    ...
                    foreach (var noChange in noChangeGroupMiners.Values)
                        stringBuilderNoChangeAlgo.Append(String.Format("{0}: {1}, ", noChange.DevicesInfoString,
                            noChange.AlgorithmType));

                    if (stringBuilderPreviousAlgo.Length > 0)
                        Helpers.ConsolePrint(TAG,
                            String.Format("Stop Mining: {0}", stringBuilderPreviousAlgo.ToString()));

                    if (stringBuilderCurrentAlgo.Length > 0)
                        Helpers.ConsolePrint(TAG,
                            String.Format("Now Mining : {0}", stringBuilderCurrentAlgo.ToString()));

                    if (stringBuilderNoChangeAlgo.Length > 0)
                        Helpers.ConsolePrint(TAG,
                            String.Format("No change  : {0}", stringBuilderNoChangeAlgo.ToString()));
                }
            }
        ...
        public async Task MinerStatsCheck(Dictionary<AlgorithmType, NiceHashSMA> NiceHashData)
        {
            ...
            try
            {
                ...
                foreach (var groupMiners in checks)
                {
                    ...
                    APIData AD = await m.GetSummaryAsync(); // LEFTOFF
                    ...
                    {
                        groupMiners.CurrentRate = NiceHashData[AD.AlgorithmID].paying * (double) AD.Speed * 0.000000001;
                    ...
                    // Update GUI
                    _mainFormRatesComunication.AddRateInfo(++index, AD, groupMiners.CurrentRate, m.IsAPIReadException);
                }
            }
            catch (Exception e)
            {
        ...
        public List<MiningDevice> GetMiningDevices()
        {
            return _miningDevices;
        }

        public Dictionary<string, GroupMiner> GetGroupMiners()
        {
            return _runningGroupMiners;
        }
    }
}
